...
export async function buildAllContent(root = process.cwd()): Promise<BuildContentResult> {
  ...
  for (const bookFile of bookFiles) {
    ...
    books.push(compileBookSource(authoredBook, dirname(bookFile)));
  }
...
async function importAuthoredBook(bookFile: string): Promise<AuthoredBook> {
  ...
  const module = (await tsImport(pathToFileURL(bookFile).href, import.meta.url)) as {
    default?: unknown;
  };
...
function renderAssetRegistry(assetPaths: readonly string[]): string {
  const lines = [
    ...
    ...assetPaths.map((assetPath) => `  '${assetPath}': require('../books/${assetPath}'),`),
    '} as const;',
    '',
